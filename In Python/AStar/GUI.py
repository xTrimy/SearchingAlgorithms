...
def GUIrun(start, goal, graph, heuristic, path, total_cost, cost, startSelection, goalSelection):
    print(start)
    start = startSelection.get()
    goal = goalSelection.get()
    cost = {start: 0}
    visited_nodes, path, total_costs = AStar.run(
        graph, heuristic, cost, start, goal)
    print("Path: " + str(path))
